...
describe("Extension Tests", () => {
    ...
    TestHook.setHttpImpl((opts: any, cb) => {
      ...
      setTimeout(() => {
        if (opts.json) {
          ...
          return cb(null, {
            statusCode: 200,
            body: `raw text, but here's the serialized params: ${JSON.stringify(
              opts
            )}`,
          });
        }
  ...
  describe("NPM", () => {
    it("express (no workspace, non-override)", done => {
      testContent
        .setWithSelection("const express = require('e^xpress')")
    ...
    it("moment (workspace, non-override)", done => {
      vscode.commands
        .executeCommand(
          "vscode.openFolder",
          vscode.Uri.parse(`file:${__dirname}/testdata`)
        )
        .then(() => {
          return vscode.window
            .showTextDocument(
              vscode.Uri.file(`${__dirname}/testdata/valid-require.js`)
            )
            .then(editor => {
              editor.selections = [
    ...
    it("not-a-real-module (no workspace, non-override)", done => {
      testContent
        .setWithSelection("const narm = require('not-a-rea^l-module')")
        .then(editor => {
          return vscode.commands.executeCommand("nodeReadme.showReadme");
        })
        .then(() => {
        ...
        .then(() => {
          const npmModules = TestHook.logData.filter(d =>
            d.startsWith(NpmProvider.SchemaType)
          );
    ...
    it("angular (no workspace, override)", done => {
      vscode.workspace
        ...
        .then(() => {
          return testContent.setWithSelection(
            "const angular = require('angula^r')"
          );
        })
        .then(editor => {
          return vscode.commands.executeCommand("nodeReadme.showReadme");
        ...
        .then(() => {
          ...
          const localModules = TestHook.logData.filter(d =>
            d.startsWith(LocalProvider.SchemaType)
          );
          const remoteModules = TestHook.logData.filter(d =>
            d.startsWith(RemoteProvider.SchemaType)
          );
